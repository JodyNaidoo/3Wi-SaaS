  ...
  return (
    <div style={{ padding: 24, fontFamily: 'Roboto, sans-serif' }}>
        ...
        <div>
          ...
          <div style={{ marginTop: 8 }}>
            {c.customerType === 'internal_unit'
              ? <span style={{ background: HEMPIRE.yellow, color: HEMPIRE.brown, padding: '4px 10px', borderRadius: 999, fontSize: 12 }}>
                  Intercompany · {c.internalUnitSlug}
                </span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
        <Card title="Contact">
          <Pair k="Email" v={c.primaryEmail} />
          <Pair k="Phone" v={c.primaryPhone} />
          <Pair k="Mobile" v={c.mobile} />
          <Pair k="Website" v={c.website} />
        </Card>
        <Card title="Tax & Compliance">
          <Pair k="VAT #" v={c.vatNumber} />
          <Pair k="Company Reg #" v={c.companyRegNumber} />
          <Pair k="BBBEE Level" v={c.bbbeeLevel} />
          <Pair k="Currency" v={c.currency} />
        </Card>
...
const Card = ({ title, children }: any) => (
  <div style={{ background: '#fafafa', border: '1px solid #eee', borderRadius: 4, padding: 14 }}>
    <div style={{ fontFamily: 'Ubuntu', color: HEMPIRE.brown, fontSize: 14, marginBottom: 8 }}>{title}</div>
    {children}
  </div>
);
const Pair = ({ k, v }: any) => (
