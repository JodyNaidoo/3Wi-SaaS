  ...
  return (
    <div style={{ padding: 24, fontFamily: 'Roboto, sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: 4, borderBottom: `2px solid ${HEMPIRE.brown}`, marginBottom: 16 }}>
        ...
        <div style={{ flex: 1 }} />
        <input
          placeholder="Search name, email, VAT…"
          value={q}
          onChange={e => setQ(e.target.value)}
          onKeyDown={e => { if (e.key === 'Enter') reload(); }}
          style={{ padding: '6px 10px', border: '1px solid #ccc', borderRadius: 3 }}
        />
        <button onClick={reload} style={btnBrown}>Search</button>
      </div>
      ...
      {showCreate && <CreateCustomerModal onClose={() => setShowCreate(false)} onSaved={() => { setShowCreate(false); reload(); }} />}
    </div>
  );
}
