 ...
 * Hempire brand: #3F1101 brown, #015807 green, #FDF31C yellow
 * Ubuntu (headings) + Roboto (body)
 */
...
type AgeingRow = {
  ...
  provider_unit_slug: string;
  balance_due_zar: string;
  due_date: string;
    ...
    (async () => {
      try {
        const [t, a] = await Promise.all([
          api.get('/invoices/_summary/totals'),
          api.get('/invoices/_summary/ageing'),
        ]);
        setTotals(t.data);
        setAgeing(a.data);
  ...
  return (
    <div style={{ padding: 24, fontFamily: 'Roboto, sans-serif', color: '#222' }}>
      ...
      <div style={{ color: '#666', marginBottom: 24 }}>
        Unified view across all 3Wi units · external + intercompany
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {overdueRows.length === 0 && (
              <tr><td colSpan={7} style={{ padding: 12, color: '#666' }}>No overdue invoices.</td></tr>
            )}
            {overdueRows.map(r => (
              <tr key={r.id} style={{ borderBottom: '1px solid #eee' }}>
                <Td><Link to={`/cc/billing/invoices/${r.id}`} style={{ color: HEMPIRE.green }}>{r.invoice_number}</Link></Td>
                <Td>{r.provider_unit_slug}</Td>
...
function Kpi({ label, value, tone }: { label: string; value: string; tone: 'brown'|'green'|'red'|'yellow' }) {
  const color =
    tone === 'green'  ? HEMPIRE.green  :
    tone === 'red'    ? '#b00020'      :
    tone === 'yellow' ? '#8a6d00'      :
                        HEMPIRE.brown;
  const bg =
    tone === 'yellow' ? HEMPIRE.yellow : 'white';
  return (
