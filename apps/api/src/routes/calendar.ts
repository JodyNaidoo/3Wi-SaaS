/**
 * calendar.ts — master calendar (every output scheduled across all 3Wi units)
 * Mounted at /calendar in server.ts.
 *
 * Routes:
 *   GET    /calendar                 - list (filters: from, to, unit, type, status, owner)
 *   GET    /calendar/:id             - one event
 *   POST   /calendar                 - create
 *   PATCH  /calendar/:id             - update
 *   DELETE /calendar/:id             - hard delete
 *   GET    /calendar/_summary/upcoming - upcoming events view (today + this week + this month)
 *   GET    /calendar/_export/ics     - RFC 5545 iCal export (subscribable)
 */
...
// ───────────────────────────────────────────── LIST
calendarRouter.get('/', async (req: AuthedRequest, res, next) => {
  try {
...
// ───────────────────────────────────────────── GET ONE
calendarRouter.get('/:id', async (req: AuthedRequest, res, next) => {
  try {
    const tid = req.user!.tenantId;
    const ev = await withTenant(tid, (tx) =>
...
// ───────────────────────────────────────────── UPCOMING VIEW
calendarRouter.get('/_summary/upcoming', async (req: AuthedRequest, res, next) => {
  try {
    const tid = req.user!.tenantId;
    const rows = await prisma.$queryRawUnsafe<any[]>(
      `SELECT * FROM upcoming_events WHERE tenant_id = $1::uuid LIMIT 100`, tid,
    );
    res.json(rows);
  } catch (e) { next(e); }
});
...
calendarRouter.post('/', async (req: AuthedRequest, res, next) => {
  try {
    const tid = req.user!.tenantId;
...
calendarRouter.patch('/:id', async (req: AuthedRequest, res, next) => {
  try {
    const tid = req.user!.tenantId;
    ...
    const updated = await withTenant(tid, async (tx) => {
      const exists = await tx.calendarEvent.findFirst({ where: { id: req.params.id, tenantId: tid } });
      if (!exists) return null;
      return tx.calendarEvent.update({ where: { id: req.params.id }, data: d });
    });
...
calendarRouter.delete('/:id', async (req: AuthedRequest, res, next) => {
  try {
    const tid = req.user!.tenantId;
    await withTenant(tid, (tx) =>
      tx.calendarEvent.deleteMany({ where: { id: req.params.id, tenantId: tid } }),
    );
    res.status(204).send();
  } catch (e) { next(e); }
});
...
// ───────────────────────────────────────────── iCAL EXPORT
calendarRouter.get('/_export/ics', async (req: AuthedRequest, res, next) => {
  try {
    const tid = req.user!.tenantId;
    const events = await withTenant(tid, (tx) =>
      tx.calendarEvent.findMany({
