...
export function InvoiceNew() {
  ...
  useEffect(() => { void api<any[]>('/customers').then(setCustomers); }, []);
  ...
  async function submit(e: any) {
    ...
    try {
      ...
      const data = await api<{ id: string }>('/invoices', { method: 'POST', body: JSON.stringify(payload) });
    ...
    <form onSubmit={submit} style={{ padding: 24, fontFamily: 'Roboto, sans-serif', maxWidth: 980 }}>
      ...
      {form.isIntercompany && (
        <div style={{ background: HEMPIRE.yellow, color: HEMPIRE.brown, padding: 10, borderRadius: 4, marginBottom: 16, fontSize: 13 }}>
          <strong>Intercompany invoice</strong> — will be flagged for elimination at group consolidation.
        </div>
      )}
