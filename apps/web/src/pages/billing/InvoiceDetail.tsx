...
export function InvoiceDetail() {
  ...
  async function reload() {
    try { setInv(await api<any>(`/invoices/${id}`)); }
  ...
  return (
    <div style={{ padding: 24, fontFamily: 'Roboto, sans-serif' }}>
      ...
      {showPay && <PayModal invoiceId={inv.id}
        suggestedAmount={Number(inv.balanceDueZar)} isIntercompany={inv.isIntercompany}
        onClose={() => setShowPay(false)}
        onSaved={async () => { setShowPay(false); await reload(); }} />}
    </div>
  );
}

function PayModal({ invoiceId, suggestedAmount, isIntercompany, onClose, onSaved }:
  { invoiceId: string; suggestedAmount: number; isIntercompany: boolean; onClose: () => void; onSaved: () => void | Promise<void> }) {
  ...
  async function submit(e: any) {
    ...
    try {
      await api(`/invoices/${invoiceId}/payments`, {
        method: 'POST',
        body: JSON.stringify({ ...form, amountZar: Number(form.amountZar) }),
      });
      await onSaved();
      ...
      <form onSubmit={submit} style={modalCard}>
        ...
        <Row label="Method">
          <select value={form.method} onChange={e => setForm({ ...form, method: e.target.value })} style={inp}>
            ...
            <option value="cash">Cash</option><option value="mobile">Mobile</option>
