...
export function MasterCalendar() {
  ...
  async function reload() {
    setLoading(true);
    const qs = new URLSearchParams({ from: monthStart.toISOString(), to: monthEnd.toISOString() });
    if (unit) qs.set('unit', unit);
    if (type) qs.set('type', type);
    const data = await api<Ev[]>(`/calendar?${qs}`);
    setEvents(data);
    setLoading(false);
  }
  useEffect(() => { void reload(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [cursor.getTime(), unit, type]);

  const grid = useMemo(() => {
    const firstDow = (monthStart.getDay() + 6) % 7; // Mon=0
    const start = new Date(monthStart); start.setDate(start.getDate() - firstDow);
    const days: Date[] = [];
  ...
  async function downloadIcs() {
    // Hit the protected endpoint with our bearer token, then trigger a download.
    const token = useAuthStore.getState().accessToken;
    const API = import.meta.env.VITE_API_URL ?? 'http://localhost:4000';
    const res = await fetch(`${API}/calendar/_export/ics`, { headers: token ? { Authorization: `Bearer ${token}` } : {} });
    const blob = await res.blob();
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = '3wi-calendar.ics'; a.click();
    ...
    <div style={{ padding: 24, fontFamily: 'Roboto, sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => void downloadIcs()} style={btnGhost}>Export .ics</button>
              ...
              return (
                <div key={i} style={{
                  ...
                  {dayEvents.slice(0, 4).map(e => (
                    <button key={e.id} onClick={() => setSelected(e)} style={eventChip(e.type)}>{e.title}</button>
                  ))}
      ...
      {selected && <EventModal ev={selected} onClose={() => setSelected(null)} onChanged={async () => { setSelected(null); await reload(); }} />}
      {showCreate && <CreateEventModal onClose={() => setShowCreate(false)} onSaved={async () => { setShowCreate(false); await reload(); }} />}
    </div>
  );
}

function CreateEventModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void | Promise<void> }) {
...
function EventModal({ ev, onClose, onChanged }: { ev: Ev; onClose: () => void; onChanged: () => void | Promise<void> }) {
  async function setStatus(status: string) {
    await api(`/calendar/${ev.id}`, { method: 'PATCH', body: JSON.stringify({ status }) });
    await onChanged();
  }
  async function del() {
